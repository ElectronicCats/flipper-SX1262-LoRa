...
void setPacketParams(uint16_t packetParam1,
...
void makePaths(void* context) {
  ...
  if (!storage_simply_mkdir(model->storage_rx, PATHAPPEXT)) {
    dialog_message_show_storage_error(model->dialogs_rx,
                                      "Cannot create\napp folder");
  }
}
...
const char* const config_bw_names[] = {
    "7.81 kHz",  "10.42 kHz", "15.63 kHz", "20.83 kHz", "31.25 kHz",
    "41.67 kHz", "62.50 kHz", "125 kHz",   "250 kHz",   "500 kHz",
};
...
const char* const config_us_dr_names[] = {
    "SF10/125kHz", "SF9/125kHz",  "SF8/125kHz",  "SF7/125kHz",
    "SF8/500kHz",  "SF12/500kHz", "SF11/500kHz", "SF10/500kHz",
    "SF9/500kHz",  "SF8/500kHz",  "SF7/500kHz"};

// Data Rate configuration for EU868
const uint8_t config_eu_dr_values[] = {0x00, 0x01, 0x02, 0x03,
                                       0x04, 0x05, 0x06};

const char* const config_eu_dr_names[] = {
    "SF12/125kHz", "SF11/125kHz", "SF10/125kHz", "SF9/125kHz",
    "SF8/125kHz",  "SF7/125kHz",  "SF7/250kHz"};
...
const char* const config_txpower_names[] = {
    "30 dBm", "28 dBm", "26 dBm", "24 dBm", "22 dBm", "20 dBm",
    "18 dBm", "16 dBm", "14 dBm", "12 dBm", "10 dBm"};
...
const uint32_t config_us915_ul_channels_125k[] = {
    ...
    903700000, 903900000, 904100000, 904300000, 904500000, 904700000, 904900000,
    905100000, 905300000, 905500000, 905700000, 905900000, 906100000, 906300000,
    906500000, 906700000, 906900000, 907100000, 907300000, 907500000, 907700000,
    907900000, 908100000, 908300000, 908500000, 908700000, 908900000, 909100000,
    909300000, 909500000, 909700000, 909900000, 910100000, 910300000, 910500000,
    910700000, 910900000, 911100000, 911300000, 911500000, 911700000, 911900000,
    912100000, 912300000, 912500000, 912700000, 912900000, 913100000, 913300000,
    913500000, 913700000, 913900000, 914100000, 914300000, 914500000, 914700000,
    914900000};
...
const uint32_t config_eu868_ul_channels_125k[] = {868100000, 868300000,
                                                  868500000};
...
const uint32_t config_eu868_dl_channels_rx1[] = {868100000, 868300000,
                                                 868500000};
...
static void lora_config_header_type_change(VariableItem* item) {
  ...
  // Order is preamble, header type, packet length, CRC, IQ
  setPacketParams(app->packetPreamble, app->packetHeaderType,
                  app->packetPayloadLength, app->packetCRC,
                  app->packetInvertIQ);
}
...
static void lora_config_us915_ul_channels_125k_change(VariableItem* item) {
  ...
  char text_buf[11] = {0};
  snprintf(text_buf, sizeof(text_buf), "%3lu.%1lu MHz",
...
static void lora_config_region_change(VariableItem* item) {
  ...
  if (index == 0) {
    ...
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_region_label,
        COUNT_OF(config_region_values), lora_config_region_change, app);
    uint8_t config_region_index = 0;
    variable_item_set_current_value_index(item, config_region_index);
    variable_item_set_current_value_text(
        item, config_region_names[config_region_index]);

    // EU868 Data Rate
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_eu_dr_label,
        COUNT_OF(config_eu_dr_values), lora_config_eu_dr_change, app);
    uint8_t config_eu_dr_index = 0;
    variable_item_set_current_value_index(item, config_eu_dr_index);
    variable_item_set_current_value_text(
        item, config_us_dr_names[config_eu_dr_index]);

    // Uplink EU868 Channel 125K
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_eu868_ul_channels_125k_label,
        COUNT_OF(config_eu868_ul_channels_125k),
        lora_config_eu868_ul_channels_125k_change, app);
    uint8_t config_eu868_ul_channels_125k_index = 0;
    variable_item_set_current_value_index(item,
                                          config_eu868_ul_channels_125k_index);
    ...
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_eu868_ul_channels_250k_label,
        COUNT_OF(config_eu868_ul_channels_250k),
        lora_config_eu868_ul_channels_250k_change, app);
    uint8_t config_eu868_ul_channels_250k_index = 0;
    variable_item_set_current_value_index(item,
                                          config_eu868_ul_channels_250k_index);
    ...
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_eu868_dl_channels_rx1_label,
        COUNT_OF(config_eu868_dl_channels_rx1),
        lora_config_eu868_dl_channels_rx1_change, app);
    uint8_t config_eu868_dl_channels_rx1_index = 0;
    variable_item_set_current_value_index(item,
                                          config_eu868_dl_channels_rx1_index);
    ...
    // US915 Data Rate
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_us_dr_label,
        COUNT_OF(config_us_dr_values), lora_config_us_dr_change, app);
    uint8_t config_us_dr_index = 0;
    variable_item_set_current_value_index(item, config_us_dr_index);
    variable_item_set_current_value_text(
        item, config_us_dr_names[config_us_dr_index]);

    // Uplink US915 Channel 125K
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_us915_ul_channels_125k_label,
        COUNT_OF(config_us915_ul_channels_125k),
        lora_config_us915_ul_channels_125k_change, app);
    uint8_t config_us915_ul_channels_125k_index = 0;
    variable_item_set_current_value_index(item,
                                          config_us915_ul_channels_125k_index);
    ...
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_us915_ul_channels_500k_label,
        COUNT_OF(config_us915_ul_channels_500k),
        lora_config_us915_ul_channels_500k_change, app);
    uint8_t config_us915_ul_channels_500k_index = 0;
    variable_item_set_current_value_index(item,
                                          config_us915_ul_channels_500k_index);
    ...
    item = variable_item_list_add(
        app->variable_item_list_lorawan, config_us915_dl_channels_500k_label,
        COUNT_OF(config_us915_dl_channels_500k),
        lora_config_us915_dl_channels_500k_change, app);
...
static void lora_config_freq_text_updated(void* context) {
  ...
  bool redraw = true;
  with_view_model(
      app->view_sniffer, LoRaSnifferModel * model,
      {
        furi_string_set(model->config_freq_name, app->temp_buffer);
        variable_item_set_current_value_text(
            app->config_freq_item,
...
static void lora_setting_item_clicked(void* context, uint32_t index) {
  ...
  if (index == 1) {
    ...
    text_input_set_result_callback(
        app->frequency_input, lora_config_freq_text_updated, app,
        app->temp_buffer, app->temp_buffer_size, clear_previous_text);

    // Pressing the BACK button will reload the configure screen.
    view_set_previous_callback(text_input_get_view(app->frequency_input),
                               lora_navigation_configure_callback);

    // Show text input dialog.
    view_dispatcher_switch_to_view(app->view_dispatcher,
                                   LoRaViewFrequencyInput);
  }
}
...
static void lora_view_sniffer_draw_callback(Canvas* canvas, void* model) {
  ...
  if (bytesRead > -1) {
    ...
    if (flag_file) {
      ...
      snprintf(time_string, TIME_LEN, CLOCK_TIME_FORMAT, curr_dt.hour,
               curr_dt.minute, curr_dt.second);
      snprintf(date_string, DATE_LEN, CLOCK_ISO_DATE_FORMAT, curr_dt.year,
               curr_dt.month, curr_dt.day);
      ...
      snprintf(
          ...
          date_string, time_string, freq_str,
          config_bw_names[my_model->config_bw_index],
          config_sf_names[my_model->config_sf_index], getRSSI(),
  ...
  furi_string_printf(xstr, "FQ:%s MHz",
                     furi_string_get_cstr(my_model->config_freq_name));
...
static void lora_view_sniffer_enter_callback(void* context) {
  ...
  app->timer_rx = furi_timer_alloc(lora_view_sniffer_timer_callback,
                                   FuriTimerTypePeriodic, context);
...
static void lora_view_transmitter_enter_callback(void* context) {
  ...
  app->timer_tx = furi_timer_alloc(lora_view_transmitter_timer_callback,
                                   FuriTimerTypePeriodic, context);
 ...
 * @param      context  The context - LoRaApp object.
 */
static bool lora_view_sniffer_custom_event_callback(uint32_t event,
                                                    void* context) {
  LoRaApp* app = (LoRaApp*) context;
  switch (event) {
    case LoRaEventIdRedrawScreen:
      // Redraw screen by passing true to last parameter of with_view_model.
      {
        bool redraw = true;
        with_view_model(
            app->view_sniffer, LoRaSnifferModel * _model, { UNUSED(_model); },
            redraw);
        return true;
      }
    case LoRaEventIdOkPressed:
      ...
      if (furi_hal_speaker_acquire(500)) {
        ...
        with_view_model(
            app->view_sniffer, LoRaSnifferModel * model,
            { frequency = model->x * 100 + 100; }, redraw);
 ...
 * @param      context  The context - LoRaApp object.
 */
static bool lora_view_transmitter_custom_event_callback(uint32_t event,
                                                        void* context) {
  LoRaApp* app = (LoRaApp*) context;
  switch (event) {
    case LoRaEventIdRedrawScreen:
      // Redraw screen by passing true to last parameter of with_view_model.
      {
        bool redraw = true;
        with_view_model(
            app->view_transmitter, LoRaTransmitterModel * _model,
            { UNUSED(_model); }, redraw);
        return true;
      }
    case LoRaEventIdOkPressed:
      // Process the OK button.  We play a tone based on the x coordinate.
      if (furi_hal_speaker_acquire(500)) {
        ...
        bool redraw = false;
        with_view_model(
            app->view_transmitter, LoRaTransmitterModel * model,
            { frequency = model->x * 100 + 100; }, redraw);
        furi_hal_speaker_start(frequency, 1.0);
        furi_delay_ms(100);
        furi_hal_speaker_stop();
      ...
      with_view_model(
          app->view_sniffer, LoRaSnifferModel * model,
          {
            ...
            if (model->flag_file) {
              ...
              if (!storage_file_open(model->file_rx, filename, FSAM_WRITE,
                                     FSOM_CREATE_ALWAYS)) {
...
void send_data(void* context) {
  ...
  dialog_file_browser_set_basic_options(&browser_options,
                                        LORA_LOG_FILE_EXTENSION, NULL);
  browser_options.base_path = PATHAPP;

  dialog_file_browser_show(model->dialogs_tx, selected_filepath,
                           predefined_filepath, &browser_options);

  if (storage_file_open(model->file_tx, furi_string_get_cstr(selected_filepath),
 ...
 * @return     true if the event was handled, false otherwise.
 */
static bool lora_view_transmitter_input_callback(InputEvent* event,
                                                 void* context) {
  LoRaApp* app = (LoRaApp*) context;
...
static void lora_app_config_set_payload_length(VariableItem* item) {
  ...
  byte_input_set_result_callback(app->byte_input, set_value, NULL, app,
                                 app->byte_buffer, app->byte_buffer_size);
...
static LoRaApp* lora_app_alloc() {
  ...
  submenu_add_item(app->submenu, "Config", LoRaSubmenuIndexConfigure,
                   lora_submenu_callback, app);
  submenu_add_item(app->submenu, "LoRaWAN", LoRaSubmenuIndexLoRaWAN,
                   lora_submenu_callback, app);
  submenu_add_item(app->submenu, "Sniffer", LoRaSubmenuIndexSniffer,
                   lora_submenu_callback, app);
  submenu_add_item(app->submenu, "Transmitter", LoRaSubmenuIndexTransmitter,
                   lora_submenu_callback, app);
  submenu_add_item(app->submenu, "Send LoRa byte", LoRaSubmenuIndexManualTX,
                   lora_submenu_callback, app);
  submenu_add_item(app->submenu, "About", LoRaSubmenuIndexAbout,
                   lora_submenu_callback, app);
  view_set_previous_callback(submenu_get_view(app->submenu),
                             lora_navigation_exit_callback);
  view_dispatcher_add_view(app->view_dispatcher, LoRaViewSubmenu,
                           submenu_get_view(app->submenu));
  view_dispatcher_switch_to_view(app->view_dispatcher, LoRaViewSubmenu);

  app->frequency_input = text_input_alloc();
  view_dispatcher_add_view(app->view_dispatcher, LoRaViewFrequencyInput,
                           text_input_get_view(app->frequency_input));
  ...
  view_dispatcher_add_view(app->view_dispatcher, LoRaViewByteInput,
  ...
  view_set_previous_callback(byte_input_get_view(app->byte_input),
  ...
  variable_item_set_current_value_text(app->config_freq_item,
                                       furi_string_get_cstr(config_freq_name));

  // bw
  item = variable_item_list_add(app->variable_item_list_config, config_bw_label,
                                COUNT_OF(config_bw_values),
                                lora_config_bw_change, app);
  ...
  // sf
  item = variable_item_list_add(app->variable_item_list_config, config_sf_label,
                                COUNT_OF(config_sf_values),
                                lora_config_sf_change, app);
  ...
  // cr
  item = variable_item_list_add(app->variable_item_list_config, config_cr_label,
                                COUNT_OF(config_cr_values),
                                lora_config_cr_change, app);
  ...
  // Payload length
  item =
      variable_item_list_add(app->variable_item_list_config, "Payload length",
                             64, lora_app_config_set_payload_length, app);
  variable_item_set_current_value_index(item, 16);
  variable_item_set_current_value_text(item, "16");

  // Header Type
  item = variable_item_list_add(
      app->variable_item_list_config, config_header_type_label,
      COUNT_OF(config_header_type_values), lora_config_header_type_change, app);
  uint8_t config_header_type_index = 0;
  variable_item_set_current_value_index(item, config_header_type_index);
  variable_item_set_current_value_text(
      item, config_header_type_names[config_header_type_index]);

  // CRC
  item = variable_item_list_add(app->variable_item_list_config,
                                config_crc_label, COUNT_OF(config_crc_values),
                                lora_config_crc_change, app);
  uint8_t config_crc_index = 0;
  variable_item_set_current_value_index(item, config_crc_index);
  variable_item_set_current_value_text(item,
                                       config_crc_names[config_crc_index]);

  // Inverted IQ
  item = variable_item_list_add(app->variable_item_list_config, config_iq_label,
                                COUNT_OF(config_iq_values),
                                lora_config_iq_change, app);
  ...
  variable_item_set_current_value_index(item, config_us_dr_index);
  variable_item_set_current_value_text(item,
                                       config_us_dr_names[config_us_dr_index]);

  char text_buf[11] = {0};

  // Uplink Channel 125K
  item = variable_item_list_add(app->variable_item_list_lorawan,
  ...
  snprintf(text_buf, sizeof(text_buf), "%3lu.%1lu MHz",
           config_us915_ul_channels_500k[config_us915_ul_channels_500k_index] /
               1000000,
           (config_us915_ul_channels_500k[config_us915_ul_channels_500k_index] %
            1000000) /
               100000);
  ...
  snprintf(text_buf, sizeof(text_buf), "%3lu.%1lu MHz",
           config_us915_dl_channels_500k[config_us915_dl_channels_500k_index] /
               1000000,
           (config_us915_dl_channels_500k[config_us915_dl_channels_500k_index] %
            1000000) /
               100000);
  ...
  view_dispatcher_add_view(
      app->view_dispatcher, LoRaViewConfigure,
      variable_item_list_get_view(app->variable_item_list_config));
  ...
  view_set_previous_callback(app->view_sniffer,
                             lora_navigation_submenu_callback);
  ...
  view_set_custom_callback(app->view_sniffer,
                           lora_view_sniffer_custom_event_callback);
  view_allocate_model(app->view_sniffer, ViewModelTypeLockFree,
                      sizeof(LoRaSnifferModel));
  ...
  view_dispatcher_add_view(app->view_dispatcher, LoRaViewSniffer,
                           app->view_sniffer);

  app->view_transmitter = view_alloc();
  view_set_draw_callback(app->view_transmitter,
                         lora_view_transmitter_draw_callback);
  view_set_input_callback(app->view_transmitter,
                          lora_view_transmitter_input_callback);
  view_set_previous_callback(app->view_transmitter,
                             lora_navigation_submenu_callback);
  view_set_enter_callback(app->view_transmitter,
                          lora_view_transmitter_enter_callback);
  view_set_exit_callback(app->view_transmitter,
                         lora_view_transmitter_exit_callback);
  view_set_context(app->view_transmitter, app);
  view_set_custom_callback(app->view_transmitter,
                           lora_view_transmitter_custom_event_callback);
  view_allocate_model(app->view_transmitter, ViewModelTypeLockFree,
                      sizeof(LoRaTransmitterModel));
  ...
  view_dispatcher_add_view(app->view_dispatcher, LoraViewTransmitter,
  ...
  view_set_previous_callback(widget_get_view(app->widget_about),
                             lora_navigation_submenu_callback);
  view_dispatcher_add_view(app->view_dispatcher, LoRaViewAbout,
