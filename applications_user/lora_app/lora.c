...
uint32_t getFreqInt() {
  ...
  floattemp =
      ((MsbH * 0x1000000ul) + (MsbL * 0x10000ul) + (Mid * 0x100ul) + Lsb);
...
void updateModulationParameters() {
  ...
  if (furi_hal_spi_bus_tx(
    ...
    FURI_LOG_E(TAG,
               "FAILED - furi_hal_spi_bus_tx or furi_hal_spi_bus_rx failed.");
    furi_hal_spi_release(spi);
  }
...
void configureRadioEssentials() {
  ...
  spiBuff[1] =
      0x04;  // paDutyCycle. See datasheet, set in conjunction with hpMax
  spiBuff[2] =
      0x07;  // hpMax. Basically Tx power. 0x00-0x07 where 0x07 is max power
  ...
  spiBuff[1] =
      0x00;  // Number of symbols. Ping-pong example from Semtech uses 5
...
bool waitForRadioCommandCompletion(uint32_t timeout) {
  ...
  while (!dataTransmitted) {
    ...
    // Parse the status
    uint8_t chipMode =
        (spiBuff[1] >> 4) & 0x07;  // Chip mode is bits [6:4] (3-bits)
    uint8_t commandStatus =
        (spiBuff[1] >> 1) & 0x07;  // Command status is bits [3:1] (3-bits)
...
bool configSetSpreadingFactor(int sf) {
  ...
  lowDataRateOptimize =
      (sf >= 11) ? 1 : 0;  // Turn on for SF11+SF12, turn off for anything else
  spreadingFactor = sf;
  updateModulationParameters();
  return true;
}
...
void setPacketParams(uint16_t packetParam1,
  ...
  spiBuff[3] =
      packetParam2;  // Header Type. 0x00 = Variable Len, 0x01 = Fixed Length
...
void setModeReceive() {
  ...
  spiBuff[6] =
      0x00;  // PacketParam6 = Invert IQ.  0x00 = Standard, 0x01 = Inverted
...
void transmit(uint8_t* data, int dataLen) {
  ...
  waitForRadioCommandCompletion(
      100);  // Give time for radio to process the command
  ...
  furi_hal_gpio_write(pin_nss1, true);  // Disable radio chip-select
  waitForRadioCommandCompletion(
      1000);  // Give time for radio to process the command
...
int lora_receive_async(uint8_t* buff, int buffMaxLen) {
  ...
  while (furi_hal_gpio_read(pin_dio1)) {
    ...
    spiBuff[1] =
        0xFF;  // IRQ bits to clear (MSB) (0xFFFF means clear all interrupts)
  ...
  rssi =
      -((int) spiBuff[2]) / 2;  // "Average over last packet received of RSSI.
  ...
  uint8_t startAddress =
      spiBuff[3];  // Where in 1262 memory is the packet stored
  ...
  furi_hal_spi_bus_rx(spi, buff, payloadLen,
