...
static void lora_relay_reload_config(LoRaRelay* instance) {
  FURI_LOG_I(TAG, "frequency: %d, sf: %d, bw: %d", instance->config_frequency,
             instance->config_sf, instance->config_bw);
}
...
LoRaRelay* lora_relay_alloc() {
  ...
  view_dispatcher_add_view(instance->view_dispatcher, LoRaRelayViewConfigure,
                           view);
  ...
  view_set_previous_callback(view, lora_relay_previous_callback);
  view_dispatcher_add_view(instance->view_dispatcher, LoRaRelayViewLoRaRX,
                           view);
  ...
  view_set_previous_callback(view, lora_relay_previous_callback);
  view_dispatcher_add_view(instance->view_dispatcher, LoRaRelayViewLoRaTX,
                           view);
  ...
  // Frequency
  item = variable_item_list_add(instance->variable_item_list, "Frequency:", 64,
                                lora_config_set_frequency, instance);
  ...
  item = variable_item_list_add(instance->variable_item_list,
                                "Band Width:", COUNT_OF(config_bw_value),
                                lora_config_set_bw, instance);
  variable_item_set_current_value_index(item, 7);
  variable_item_set_current_value_text(item, config_bw_text[7]);
  // Spread Factor
  item = variable_item_list_add(instance->variable_item_list,
                                "Spread Factor:", COUNT_OF(config_sf_value),
                                lora_config_set_sf, instance);
  ...
  view_dispatcher_add_view(instance->view_dispatcher, LoRaRelayViewSubmenu,
                           view);
  submenu_add_item(instance->submenu, "LoRa settings", LoRaRelayViewConfigure,
                   lora_relay_submenu_callback, instance);
  submenu_add_item(instance->submenu, "LoRa sniffer", LoRaRelayViewLoRaRX,
                   lora_relay_submenu_callback, instance);
  submenu_add_item(instance->submenu, "LoRa sender", LoRaRelayViewLoRaTX,
                   lora_relay_submenu_callback, instance);
  submenu_add_item(instance->submenu, "About", LoRaRelayViewLoRaAbout,
                   lora_relay_submenu_callback, instance);

  instance->widget_about = widget_alloc();
  widget_add_text_scroll_element(
      instance->widget_about, 0, 0, 128, 64,
      "This is a sample application.\n---\nReplace code and message\nwith your "
      "content!\n\nauthor: "
      "@codeallnight\nhttps://discord.com/invite/NsjCvqwPAd\nhttps://"
      "youtube.com/@MrDerekJamison");
  view_set_previous_callback(widget_get_view(instance->widget_about),
                             lora_relay_previous_callback);
  view_dispatcher_add_view(instance->view_dispatcher, LoRaRelayViewLoRaAbout,
