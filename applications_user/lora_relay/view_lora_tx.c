...
static bool view_lora_tx_input_callback(InputEvent* event, void* context) {
  ...
  if (event->type == InputTypeShort || event->type == InputTypeRepeat) {
    with_view_model(
        instance->view, ViewLoRaTXModel * model,
        {
          if (event->key == InputKeyLeft && model->test > 0) {
            ...
            FuriString* predefined_filepath =
                furi_string_alloc_set_str(PATHAPP);
            ...
            dialog_file_browser_show(model->dialogs, selected_filepath,
                                     predefined_filepath, &browser_options);

            if (storage_file_open(model->file,
              ...
              dialog_message_show_storage_error(model->dialogs,
                                                "Cannot open File");
            }
...
ViewLoRaTX* view_lora_tx_alloc() {
  ...
  instance->timer = furi_timer_alloc(view_lora_tx_timer_callback,
                                     FuriTimerTypePeriodic, instance);
