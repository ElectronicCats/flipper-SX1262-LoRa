...
static void view_lora_rx_draw_callback_move(Canvas* canvas, void* _model) {
  ...
  canvas_draw_str(canvas, 0, 62,
                  (const char*) receiveBuff);  //(char*)receiveBuff);
}
...
static bool view_lora_rx_input_callback(InputEvent* event, void* context) {
  ...
  if (event->type == InputTypeShort || event->type == InputTypeRepeat) {
    with_view_model(
        instance->view, ViewLoRaRXModel * model,
        {
          if (event->key == InputKeyLeft && model->test > 0) {
            ...
            if (model->flag_file) {
              storage_file_open(model->file, PATHLORA, FSAM_WRITE,
                                FSOM_CREATE_ALWAYS);
...
ViewLoRaRX* view_lora_rx_alloc() {
  ...
  instance->timer = furi_timer_alloc(view_lora_rx_timer_callback,
